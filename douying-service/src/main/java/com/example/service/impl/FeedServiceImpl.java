...
public class FeedServiceImpl implements FeedService {
    ...
    @Override
    @Async
    public void pusOutBoxFeed(Long userId, Long videoId, Long time) { //推入收件箱
        redisCacheUtil.zadd(RedisConstant.OUT_FOLLOW + userId, time, videoId, -1); //放入发件箱中
    }

    //todo
    @Override
    public void pushInBoxFeed(Long userId, Long videoId, Long time) { //拉取到收件箱
        redisCacheUtil.zadd(RedisConstant.IN_FOLLOW + userId, time, videoId, -1);
    }

    @Override
    @Async
    public void deleteOutBoxFeed(Long userId,Collection<Long> fans,Long videoId) { //删除发件箱
        String t = RedisConstant.IN_FOLLOW;
        redisTemplate.executePipelined((RedisCallback<Object>) connection -> { //将关注的粉丝中的收件箱的视频ID全部删除
            for (Long fan : fans) {
                connection.zRem((t+fan).getBytes(),String.valueOf(videoId).getBytes());
            }
            connection.zRem((RedisConstant.OUT_FOLLOW + userId).getBytes(), String.valueOf(videoId).getBytes()); //移除发送的视频
            return null;
        });
    }
    ...
    @Override
    @Async
    public void deleteInBoxFeed(Long userId,List<Long> videoIds) { //删除收件箱
    ...
    public void initFollowFeed(Long userId,Collection<Long> followIds) {
        ...
        final Date limitDate = DateUtil.addDateDays(curDate, -7);//当前时间的前7天
